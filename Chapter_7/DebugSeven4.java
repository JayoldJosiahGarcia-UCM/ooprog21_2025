...
public class DebugSeven4
{
    public static void main(String[] args)
    {
        ...
        for(int i = 0; i < stringLength; i++) // remove = operator
        {
            char ch = Character.toLowerCase(aString.charAt(i)); //added a . between aStringcharAt(i)
            System.out.print(ch);
            if(Character.isLetter(ch)) // remove ! operator
                numLetters++;
        }
        System.out.println();
        System.out.println("The number of CHARACTERS in the string is " + stringLength);
        System.out.println("The number of LETTERS is " + numLetters); //added a closing quatation mark 
    }
}
